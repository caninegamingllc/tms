...
export type PayLineTypeOption = {
  id: string;
  name: string;
  calculationMethod: CarrierPayCalculationMethod | string;
};

export type InitialPayLine = {
  lineTypeId: string;
...
export function CarrierPayLinesEditor({
  lineTypes,
  initialLines,
  defaultMiles
}: {
  lineTypes: PayLineTypeOption[];
  initialLines?: InitialPayLine[];
  ...
  if (!lineTypes.length) {
    return (
      <p className="text-sm text-muted-foreground">
        No carrier pay line types configured. An admin can add them under Admin → Settings.
      </p>
    );
  }
    ...
    <div className="grid gap-3">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <p className="text-sm font-semibold text-foreground">
          Carrier pay total: {formatMoney(totalCents)}
        </p>
      </div>
    </div>
  );
